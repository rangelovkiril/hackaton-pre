/**
 * examples.ts — mqtt-hub + Elysia WebSocket.
 *
 *     bun run examples.ts
 *
 * Test:
 *     mosquitto_pub -t "sensors/room/temp" -m '{"value": 23.5}'
 *     bunx wscat -c ws://localhost:3000/sensors
 */
...
hub.on('binary/imu', parseIMU, (_topic, [ax, ay, az]) => {
  console.log(`[imu]  ax=${ax.toFixed(2)} ay=${ay.toFixed(2)} az=${az.toFixed(2)}`)
})

// ── Outgoing: periodic heartbeat ─────────────────────────────────────────

setInterval(() => {
  hub.publish('status/heartbeat', JSON.stringify({ alive: true, ts: Date.now() }))
}, 5000)
